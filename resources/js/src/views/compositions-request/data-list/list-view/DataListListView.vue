  ...
  <div id="data-list-list-view" class="data-list-container">
    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="dataObjects">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center data-list-btn-container">
          ...
          <div class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary" @click="addNewData">
              ...
              <span class="ml-2 text-base text-primary">បង្កើតសំណើរ</span>
          </div>
        </div>
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="name">ចំនងជើងសំណើរ</vs-th>
        <vs-th sort-key="role">សំណើរពី</vs-th>
        <vs-th sort-key="bank">ថ្ងៃបង្កើតសំណើរ</vs-th>
        <!-- <vs-th sort-key="bank_acc">ចំនួនមន្ត្រីដោះដូរ</vs-th> -->
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate" @click="addNewData">{{ tr.request_title }}</p>
              </vs-td>

              <vs-td>
                <p class="product-role" @click="addNewData">{{ tr.request_from }}</p>
              </vs-td>

              <vs-td>
                <p class="product-role" @click="addNewData">{{ tr.request_date }}</p>
              </vs-td>

              <!-- <vs-td>
                <vs-chip :color="getOrderStatusColor(tr.order_status)" class="product-order-status" @click="addNewData">{{ tr.bank_acc_no }}</vs-chip>
              </vs-td> -->
...
export default {
  ...
  methods: {
    addNewData() {
      this.$router.push('/incentive/composition-request/form').catch(() => {})
      // this.sidebarData = {}
    ...
    editData(data) {
        this.$router.push('/incentive/composition-request/edit/' + data.id).catch(() => {})

  ...
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      <div class="p-6">
        ...
        <vs-input type="number" label="Incentive" v-model="incentive" class="mt-5 w-full" name="incentive" v-validate="'required'" />
        <span class="text-danger text-sm" v-show="errors.has('incentive')">{{ errors.first('incentive') }}</span>
  ...
  watch: {
    isSidebarActive(val) {
      ...
      if(Object.entries(this.data).length === 0) {
        ...
        let { id, name, incentive, order_level } = JSON.parse(JSON.stringify(this.data))
        this.dataId = id
        this.dataName = name
      ...
      this.$validator.validateAll().then(result => {
          if (result) {
            ...
            if(this.dataId !== null && this.dataId >= 0) {
              ...
              this.$store.dispatch("dataList/updateDuty", obj).catch(err => { console.error(err) })
            }else{
              delete obj.id
              obj.popularity = 0

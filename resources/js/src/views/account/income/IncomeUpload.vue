  ...
  <vx-card no-shadow>
    ...
    <form>
      ...
      <div class="flex flex-wrap items-center mb-base">
        <vs-avatar :src="fileUrl" size="70px" class="mr-4 mb-4" />
        <div class="vx-col md:w-1/1 w-full mt-5">
          <!-- <vs-file-input accept="image/*" class="mr-4 sm:mb-0 mb-2">Upload File</vs-file-input> -->
          <!-- <v-file-input class="mr-4 sm:mb-0 mb-2" accept="image/*" label="File input"></v-file-input> -->
          ...
          <p class="text-sm mt-2">Allowed Only(.xlsx) <a target="_blank" href="/sample/sample-income.xlsx">Download Sample</a></p>
      ...
      <div class="vx-row">
        ...
        <div class="vx-col md:w-1/2 w-full mt-5">
          <label for="" class="vs-input--label">ថ្ងៃទទួលចំណូល</label>
          <datepicker v-validate="'required'" placeholder="ថ្ងៃទទួលចំណូល" name="issue_date" v-model="issue_date"></datepicker>
...
const dict = {
    custom: {
        ...
        issue_date: {
          required: 'សូមជ្រើសថ្ងៃដែលបានទទួល',
          alpha: "First name may only contain alphabetic characters"
        },
        category: {
          required: 'សូមជ្រើសរើសប្រភេទចំនូល',
          alpha: "First name may only contain alphabetic characters"
        },
        currency: {
...
export default {
  ...
  data() {
    return {
      file: '',
      fileUrl: "/images/file-icons/xls.png",
      title: "",
      account: "",
      description: "",
      issue_date: "",
    ...
    saveGeneral(){
      this.$validator.validateAll().then(result => {
        if(result){
          ...
          return new Promise((resolve, reject) => {
              axios.post("/api/v1/file_income_upload", formData)
                  .then((response) => {
                  ...
                  this.$router.push('/account/income').catch(() => {})
              }).catch((error) => { 
                reject(error)
                this.$vs.notify({
                  title: 'Message',
      ...
      //       let _id = this.$route.params.id;
      //       return new Promise((resolve, reject) => {
      //           axios.put("/api/v1/income/" + _id, _data)
      //               .then((response) => {
      //               this.$vs.notify({
      //                 title: 'Message',
      //                 text: response.data.message,
      ...
      //     }else{
      //       return new Promise((resolve, reject) => {
      //           axios.post("/api/v1/income", _data)
      //               .then((response) => {

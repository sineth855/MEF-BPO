...
class CeilingGroup extends Model
{
  ...
  public static function getCeilingGroupsOptions($filter){
    $data = array();
    $query = CeilingGroup::orderBy($filter["sort"], $filter["order"]);
    $whereClause = $query;
  ...
  public static function getCeilingGroups($filter){
    $data = array();
    $query = CeilingGroup::orderBy($filter["sort"], $filter["order"]);
    $whereClause = $query;
    $whereClause->whereNotIn("status", [4])->orWhereNull("status");

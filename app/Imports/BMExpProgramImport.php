...
use App\Models\Modules\BudgetMonitoring\ExpProgram;
use Maatwebsite\Excel\Concerns\ToModel;
use Maatwebsite\Excel\Concerns\WithHeadingRow;
use DB;
use Auth;
class BMExpProgramImport implements ToModel,WithHeadingRow
{
    /**
    * @param array $row
    *
    ...
    */
    public function model(array $row)
    {
        // dd($row["id"]);
        $id = $row["id"];
        $queryCheck = ExpProgram::where("id", $id)->first();
        if(!$queryCheck){
            if($row["entity_id"] != ""){
        ...
        $queryDetailCheck = DB::table("mef_expenditure_program_detail")
                            ->where("id", (int)$row["id"])
                            ->first();
        if(!$queryDetailCheck){
            if($row["exp_pro_id"] !="")
                DB::table("mef_expenditure_program_detail")->insert([
                    "id"=>$row["id"],
                    "exp_pro_id"=>$row["exp_pro_id"],

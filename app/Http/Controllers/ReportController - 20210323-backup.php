...
class ReportController extends Controller
{
    ...
    public function summary(Request $request){
        ...
        // $dddd = Income::select(DB::raw('YEAR(DATE_SUB("'.$filter.'", INTERVAL 1 YEAR)) as year'))->whereRaw('year(`issue_date`) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 YEAR))')->first();       
        ...
        $quarterExpenseThisYear = DB::table("incentive_to_officer")->SELECT(DB::raw("QUARTER(`issue_to_date`) as quarter, (sum(incentive) + sum(reminder_amount)) as amount"))
                                                              ->groupBy(DB::raw("QUARTER(`issue_to_date`)"))
                                                              ->whereRaw('year(`issue_to_date`) = ?', array($year))
                                                              ->get();
    ...
    public function getIncomeOutcomeByQuarter(Request $request){
        ...
        $getLast12MonthExpenses = IncentiveToOfficer::whereRaw('year(`issue_from_date`) = ?', array($year))
                        // ->where('issue_date', "<=", NOW())
    ...
    public function convertAnnexLMToExcel(Request $request){
        ...
        $html = view('template/AnnexToLM');
        $filename = "./upload/".date("Ymd")."_annex_lm.xls";
        file_put_contents($filename, $html);
    ...
    public function convertAnnexBankToExcel(Request $request){
        ...
        $html = view('template/AnnexToBank');
        $filename = "./upload/".date("Ymd")."_annex_bank_".$bankName.".xls";
        file_put_contents($filename, $html);
    ...
    public function convertAnnexGDNTToExcel(Request $request){
        ...
        $html = view('template/AnnexToGDNT');
        $filename = "./upload/".date("Ymd")."_annex_GDNT.xls";
        file_put_contents($filename, $html);
